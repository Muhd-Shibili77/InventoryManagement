...
export const addItems = createAsyncThunk(
  ...
  async ({ newItem }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post(`/item/add`, newItem, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
...
export const updateItem = createAsyncThunk(
  ...
  async ({ id,editItem }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.put(`/item/update?id=${id}`, editItem, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
...
export const deleteItems = createAsyncThunk(
  ...
  async ({ itemId,isDelete }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.delete(`/item/delete?id=${itemId}&isDelete=${!isDelete}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
